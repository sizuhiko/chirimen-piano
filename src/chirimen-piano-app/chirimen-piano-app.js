...
class ChirimenPianoApp extends GestureEventListeners(PolymerElement) {
  ...
  _fromPage(page) {
    // key 0 は標準のド
    var hz = function(key) { return 442 * Math.pow(2, (1 / 12) * (key - 9)); };
    var keys = [
      [{name: 'ド', hz: hz(-12)}, {name: 'レ', hz: hz(-10)}, {name: 'ミ', hz: hz(-8)}, {name: 'ファ', hz: hz(-7)}],
      [{name: 'ソ', hz: hz(-5)}, {name: 'ラ', hz: hz(-3)}, {name: 'シ', hz: hz(-1)}, {name: 'ド', hz: hz(0)}],
      [{name: 'ド', hz: hz(0)}, {name: 'レ', hz: hz(2)}, {name: 'ミ', hz: hz(4)}, {name: 'ファ', hz: hz(5)}],
      [{name: 'ソ', hz: hz(7)}, {name: 'ラ', hz: hz(9)}, {name: 'シ', hz: hz(11)}, {name: 'ド', hz: hz(12)}],
      [{name: 'ド', hz: hz(12)}, {name: 'レ', hz: hz(14)}, {name: 'ミ', hz: hz(16)}, {name: 'ファ', hz: hz(17)}],
      [{name: 'ソ', hz: hz(19)}, {name: 'ラ', hz: hz(21)}, {name: 'シ', hz: hz(23)}, {name: 'ド', hz: hz(24)}],
    ];
    return keys[page + 2];
  }
  play(event) {
